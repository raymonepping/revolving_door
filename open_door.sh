...
NS="default"
CJ="door-open"
JOB="door-open-$(date +%s)"
URL="http://demo.local:18200/api"

# fast-path: already in desired state
if command curl -sS "$URL" | grep -q "\"door\": \"opened\""; then
  echo "✅ already opened"
  exit 0
fi
...
wait_for_door() {
  ...
  for i in $(seq 1 "$tries"); do
    ...
    if [ -z "$door" ]; then
      # likely 502/HTML/empty while restarting
...
# create job, wait, show logs, cleanup always
